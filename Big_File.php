...
<?php 
if(session_id() == ''){session_start();}
$_SESSION['activepage']='Big_File.php?id='.$_GET["id"].'&';
include('Paper.php');
?>
...
<div class="wall" id="Wall">
	...
	<div class="wall_end">
	<?php
	$prev='select WID from wallpapers where WID>'.$row_img["WID"].' and CID='.$row_img["CID"].' ORDER BY WID LIMIT 0 , 1';
	$result_prev = mysqli_query($con, $prev);
	$row_prev = mysqli_fetch_array($result_prev);
	
	$next='select WID from wallpapers where WID<'.$row_img["WID"].' and CID='.$row_img["CID"].' ORDER BY WID DESC LIMIT 0 , 1';
	...
	{
	echo '
	<a href="Big_File.php?id='.$row_prev["WID"].'" style="float:left; margin-left:50px; margin-top:12px; color:#FFF;">Previous</a>
	...
	<a href="Big_File.php?id='.$row_next["WID"].'" style="float:right; margin-right:50px; margin-top:12px; color:#FFF;">Next</a>';
	}
	else
	{
    echo '
