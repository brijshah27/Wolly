...
if(isset($_GET["msg"]))
{
	echo '<script>document.getElementById("process_text").style.display="block"</script>';
	if($_GET["msg"]=="Success_P")
	{
	 echo '<p class="text" id="text">Password is reset</p>';
	}
	else if($_GET["msg"]=="Success_M" && isset($_GET["mail"]))
	{
	 echo '<p class="text" id="text">Your Mail ID is '.$_GET["mail"].'</p>';
	}
	else if($_GET["msg"]=="Success_B" && isset($_GET["mail"]))
	{
	 echo '<p class="text" id="text">Password is reset<br /><br />Your Mail ID is '.$_GET["mail"].'</p>';
	}
}
else if(isset($_GET["pro"]))
{
	echo '<script>document.getElementById("process_text").style.display="block"</script>';
	echo '<p class="text" id="text">You Entered Data is Wrong...</p>';
}
?>
